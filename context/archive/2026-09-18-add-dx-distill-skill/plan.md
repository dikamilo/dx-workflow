...
#### Manual
- [x] 4.8 Docs written with the `documentation` skill — invoked and its Diátaxis type patterns applied: the tutorial is learning-oriented (numbered steps, a verifiable result at each, minimal conceptual digression), the explanation understanding-oriented (context → concept → trade-offs, no step-by-step), each kept to a single type and cross-linked. Its "ask clarifying questions first" step was skipped — the plan had already fixed both doc types and their subjects. Confirmed by the user 2026-09-18 — 9d53934
