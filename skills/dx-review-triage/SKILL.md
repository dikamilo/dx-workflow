...
Turn a review report's findings into decisions — and, when you say so, into edits. `dx-plan-review` and `dx-impl-review` only analyze and report; this is the one place that **acts** on a finding, editing `plan.md` or the code it reviewed, one finding at a time, only on your confirmation. That split keeps both review gates pure (root `CLAUDE.md`, DESIGN §3): a reviewer that never fixes what it checks doesn't need to graduate into a tool-restricted agent.
