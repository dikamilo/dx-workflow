...
**Say it before you call it.** State the model choice as a one-line preamble before each delegating call — e.g. "delegating drift-check to Haiku" — so the choice is visible in the transcript instead of silently baked into a tool-call parameter no one reads back. This is the only way to audit whether this policy is actually being followed run to run.
