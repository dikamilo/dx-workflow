...
- **Purpose:** hunt the codebase for design problems worth fixing — shallow modules to turn deep, plus what the wider design lenses surface (duplication, coupling, single-responsibility) — present them inline with an opinionated top pick, and promote the ones you pick; see [find refactors](../tutorials/find-refactors.md).
...
- **Writes:** findings inline as markdown — each carrying a one-line `Shape: before → after`, a strength tag (desirability), a dependency category (feasibility), and a win cashed out in `module-design` terms rather than "cleaner code" — closing with a one-sentence **top pick** (which to tackle first and why, since the strength tag ranks confidence, not sequence); ephemeral, no debt register. For a single pick, `context/changes/<slug>/change.md` stamped `type: refactor` with the finding as its seed `research/`/`frame.md`. Many picks are handed to `/dx-new` as a seed summary that carries the top pick as its closing `Start with:` line.
- **Prints next:**
  ```text
